...
struct PongMode : Mode {
	...
	struct Ball {
		Ball(glm::vec2 Position_, glm::vec2 Velocity_, float Trail_lenght_, uint32_t Type_): 
			Position(Position_), Velocity(Velocity_), Trail_length(Trail_lenght_), Type(Type_){ 
				Ball_trail.emplace_back(Position, Trail_length);
				Ball_trail.emplace_back(Position, 0.0f);
			}
		glm::vec2 Position;
		glm::vec2 Velocity;
		float Trail_length;
	...
	bool isBallCollideRightPaddle = false;
	bool isKeyBallCollideLeftPaddle = false;
	bool isEnd = false;
